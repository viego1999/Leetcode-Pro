...
[常用代码模板1](https://www.acwing.com/blog/content/277/)  
[常用代码模板2](https://www.acwing.com/blog/content/404/)  
[常用代码模板3](https://www.acwing.com/blog/content/405/)  
[常用代码模板4](https://www.acwing.com/blog/content/406/)
...
​ 一般为在给定限制条件 limit 下的【最大值最小 】或【最小值最大】

```c++
bool check(int limit, int mid) {/* ... */} // 检查m是否满足给定限制条件h下的答案

// 一般为 二分猜答案
int binary_search(int[] nums, int limit) {
    int l = 1, r = max(nums);
    while (l < r) {
        int mid = l + r >> 1;
        if (check(nums, limit, mid)) r = mid;
        else l = mid + 1;
    }
    return l;
}
...
// ============================================================================
// 例题：lc 875.爱吃香蕉的珂珂
int minEatingSpeed(int[] nums, int h) {
    int l = 0, r = max(nums);
    while (l < r) {
        int m = l + r >> 1, t = 0;
        for (int num : nums) t = (num + m - 1) / m;
        if (t > h) l = m + 1;
        else r = m;
    }
    return l;
