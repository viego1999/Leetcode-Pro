...
public class Problem2100 {
    public static void main(String[] args) {
        ...
        System.out.println(goodDaysToRobBank(new int[]{2, 1, 1, 0}, 1));
        System.out.println(goodDaysToRobBank(new int[]{0, 0, 0, 0, 2, 1}, 2));

        System.out.println(goodDaysToRobBankDp(new int[]{5, 3, 3, 3, 5, 6, 2}, 2));
     ...
     * 输出：[2,3]
     */
    public static List<Integer> goodDaysToRobBankDp(int[] security, int time) {
        List<Integer> ans = new ArrayList<>();
        boolean[] dp = new boolean[security.length];
        for (int i = time; i + time < security.length; i++) {
            boolean flag = true;
            for (int j = 0; j < time; j++) {
                if (dp[i - 1] && security[i] <= security[i - 1] &&
                        security[i] <= security[i + time] &&
                        security[i + time] >= security[i + time - 1]) break;
    ...
    public static List<Integer> goodDaysToRobBank(int[] security, int time) {
        ...
        for (int i = 1, n = security.length; i < n - 1; i++) {
