        ...
        for (int i = 0, n = code.length(); i < n; i++) {
            if (code.charAt(i) == '<') {
                if (i < n - 1) {
                    ...
                    if (Character.isUpperCase(c)) { // TAG_NAME
                        int j = i + 1;
                        while (j < n && Character.isUpperCase(code.charAt(j))) j++;
                        if (j == n || code.charAt(j) != '>' || j == i + 1 || j - i > 10) return false;
                        stack.push(code.substring(i + 1, j));
                        i = j;
                    } else if (c == '/') { // /TAG_NAME
                        int j = i + 2;
                        while (j < n && code.charAt(j) != '>') j++;
                        if (j == n || stack.isEmpty() || !stack.pop().equals(code.substring(i + 2, j))) return false;
                        if (stack.isEmpty() && j != n - 1) return false;
                        i = j;
                    } else if (c == '!') { // cdata
                        int j = i + 2;
                        if (stack.isEmpty() || j + 7 > n || !code.startsWith("[CDATA[", j)) return false;
                        j = j + 7;
                        while (j + 3 <= n && !code.startsWith("]]>", j)) j++;
                        if (j + 3 > n) return false;
                        i = j + 2;
                    } else return false;
                } else return false;
